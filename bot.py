...
# Auto role settings
AUTO_ROLE_NAME = "Members"          # Role given after authorize
...
async def give_members_role(user_id: str):
    """Give the Members role to the user in available guilds"""
    role_name = AUTO_ROLE_NAME.lower()
    ...
    for guild in bot.guilds:
        try:
            ...
            if role not in member.roles:
                ...
                print(f"Gave {AUTO_ROLE_NAME} to {user_id} in {guild.name}")
        except Exception as e:
            print(f"Failed to give role in {guild.name}: {e}")
...
async def help_command(ctx: commands.Context):
    ...
    embed.add_field(name="👥 Pool Management", value="`!addpool <user_id>` - Add user to pool\n`!removepool <user_id>` - Remove from pool\n`!backup` - Download authorized users list", inline=False)
