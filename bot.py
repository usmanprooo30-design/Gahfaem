...
MAIN_GUILD_ID = 1532047125742096394  # Guild where /claim gives the member role
...
async def help_cmd(ctx):
    ...
    embed.add_field(name="Authorization", value="`!auth` / `/auth` - Authorize yourself\n`!addbot` / `/addbot` - Invite the bot\n`!claim` / `/claim` - Claim member role (authorized only)", inline=False)
...
async def claim_cmd(ctx):
    if not await get_token(str(ctx.author.id)):
        return await ctx.send(embed=discord.Embed(
            title=f"{E['error']} Not Authorized",
            description="You must authorize first with `!auth` or `/auth` before you can claim the member role.",
    ...
    if not guild:
        try:
            ...
            return await ctx.send(embed=discord.Embed(
                title=f"{E['error']} Guild Error",
                description="Could not find the main guild. Make sure the bot is in it.",
                color=discord.Color.red()
            ))
    ...
    if not member:
        try:
            ...
            return await ctx.send(embed=discord.Embed(
                title=f"{E['error']} Not in Guild",
                description="You must be a member of the main server to claim the role.",
                color=discord.Color.red()
            ))

    role = discord.utils.get(guild.roles, name="member") or discord.utils.find(lambda r: r.name.lower() == "member", guild.roles)
    if not role:
        return await ctx.send(embed=discord.Embed(
            title=f"{E['error']} Role Not Found",
            description="The **member** role does not exist in the main server.",
            color=discord.Color.red()
        ))

    if role in member.roles:
        return await ctx.send(embed=discord.Embed(
            title=f"{E['info']} Already Have Role",
            description="You already have the **member** role.",
            color=discord.Color.gold()
        ))

    try:
        await member.add_roles(role, reason="Claimed via /claim or !claim after authorization")
        await ctx.send(embed=discord.Embed(
            title=f"{E['success']} Role Claimed",
            description=f"You have been given the {role.mention} role!",
            color=discord.Color.green()
        ))
    except discord.Forbidden:
        await ctx.send(embed=discord.Embed(
            title=f"{E['error']} Permission Error",
            description="I don't have permission to give that role (check role hierarchy & Manage Roles).",
            color=discord.Color.red()
        ))
    except Exception as e:
        await ctx.send(embed=discord.Embed(
            title=f"{E['error']} Error",
            description=str(e),
            color=discord.Color.red()
        ))
...
async def tutorial_cmd(ctx):
    ...
    embed.add_field(name="1. Authorize", value="Use `!auth` or `/auth` and complete authorization", inline=False)
    embed.add_field(name="2. Claim Role", value="Use `!claim` or `/claim` to get the member role (must be authorized)", inline=False)
    embed.add_field(name="3. Get Higher Role", value="Member / Silver / Gold / Diamond / Premium", inline=False)
    embed.add_field(name="4. Add Bot", value="Use `!addbot` in the target server", inline=False)
    embed.add_field(name="5. Farm", value="`!farm server_id`", inline=False)
    await ctx.send(embed=embed)
...
async def slash_help(i: discord.Interaction):
    embed = discord.Embed(title=f"{E['help']} Bot Help", color=discord.Color.blurple())
    embed.add_field(name="Main", value="`/auth` `/addbot` `/farm` `/pool` `/claim`", inline=False)
...
@tree.command(name="claim", description="Claim the member role (only if authorized)")
async def slash_claim(i: discord.Interaction):
    if not await get_token(str(i.user.id)):
        return await i.response.send_message(embed=discord.Embed(
            title=f"{E['error']} Not Authorized",
            description="You must authorize first with `/auth` before you can claim the member role.",
            color=discord.Color.red()
        ), ephemeral=True)
    ...
    if not member:
        try:
            member = await guild.fetch_member(i.user.id)
        except Exception:
            return await i.response.send_message(embed=discord.Embed(
                title=f"{E['error']} Not in Guild",
                description="You must be a member of the main server to claim the role.",
    ...
    try:
        await member.add_roles(role, reason="Claimed via /claim after authorization")
...
async def slash_tutorial(i: discord.Interaction):
    embed = discord.Embed(title=f"{E['info']} Tutorial", color=discord.Color.purple())
    embed.add_field(name="Steps", value="1. `/auth`\n2. `/claim` (get member role)\n3. Get higher farming role if needed\n4. `/addbot`\n5. `!farm server_id`", inline=False)
    await i.response.send_message(embed=embed)
