"""
MemberFlow - Discord Member Farm Bot
=======================================================
Features:
...
DEFAULT_EMBEDS = {
    "auth": {
        "title": "VERIFY FOR MEMBERS",
        "description": "Verifying allows you to use `!djoin` for members.\n\n**Caution:** Do not verify if you do not want to be dragged into servers.\n\n• This is fair for everyone with this\n\n**You'll receive**\n• @Members role\n• Access to farm channels\n• Member perks\n\n**Why verify?**\n• Keeps out bots & raiders\n• One click, ~5 seconds\n• No password needed",
        "color": 0x5865F2,
        "footer": "MemberFlow • Secure verification"
    },
    "help": {
        "title": "📖 MemberFlow Commands",
        "description": "Here are all available commands:",
        "color": 0x5865F2,
        "footer": "Limits → Member:2 | Silver:10 | Gold:15 | Diamond:25 | Premium:35"
    },
    "tutorial": {
        "title": "ℹ️ How to Use MemberFlow",
        "description": "Follow these simple steps:",
        "color": 0x9B59B6,
        "footer": "MemberFlow • Need help? Contact the server owner"
    },
    "pool": {
        "title": "👥 Authorization Pool",
        "description": "Users who have verified:",
        "color": 0xFEE75C,
        "footer": "MemberFlow • These users can be pulled into servers"
    }
}
...
class AuthView(View):
    ...
    @discord.ui.button(label="Verify", style=discord.ButtonStyle.green, custom_id="persistent_auth")
...
class AddBotView(View):
    ...
    @discord.ui.button(label="Add Bot", style=discord.ButtonStyle.blurple, custom_id="persistent_addbot")
...
class EmbedEditModal(Modal):
    def __init__(self, embed_name: str, current: dict):
        super().__init__(title=f"Edit: {embed_name}")
        self.embed_name = embed_name

        self.title_input = TextInput(
            label="Title",
            placeholder="Embed title (emojis allowed)",
            default=current.get("title", "")[:256],
            max_length=256,
            required=True
        )
        self.desc_input = TextInput(
            label="Description (paste animated emojis here)",
            placeholder="Full embed text. Paste <:name:id> or <a:name:id> for custom/animated emojis",
        ...
        self.footer_input = TextInput(
            ...
            placeholder="Footer text under the embed",
            default=(current.get("footer") or "")[:2048],
            max_length=2048,
            required=False
        )
        self.color_input = TextInput(
            label="Color hex (no #)",
            placeholder="Example: 5865F2 or 57F287",
...
async def help_command(ctx: commands.Context):
    ...
    embed.add_field(name="🌾 Farming", value="`!djoin <server_id>` - Farm members\n`!pool` `/pool` - See authorized count", inline=False)
    ...
    embed.add_field(name="Other", value="`!claim` - Claim Members role\n`!myauth` `!access` `!tutorial`", inline=False)
    if data.get("footer"):
...
async def claim_command(ctx: commands.Context):
    """Claim the Members role (only if authorized)"""
    if ctx.guild is None:
    ...
    if not token:
        embed = discord.Embed(
            ...
            description="You must authorize first using `!auth` before claiming the role.",
    ...
    if role is None:
        ...
        role = discord.utils.find(lambda r: r.name.lower() == "members", ctx.guild.roles)
    
    if role is None:
        embed = discord.Embed(
            title="❌ Role Not Found",
            description="Members role not found in this server.",
            color=discord.Color.red()
        )
        return await ctx.send(embed=embed)
    ...
    if role >= bot_member.top_role:
        embed = discord.Embed(
            ...
            description=f"Bot role must be **above** {role.mention} in Server Settings → Roles.",
            color=discord.Color.red()
        )
        return await ctx.send(embed=embed)

    try:
        await ctx.author.add_roles(role, reason="Claimed via !claim")
        embed = discord.Embed(
            title="✅ Role Claimed",
        ...
        embed = discord.Embed(
            title="❌ Failed",
            description=f"Could not give role: `{e}`",
            color=discord.Color.red()
        )
        await ctx.send(embed=embed)
...
async def tutorial_command(ctx: commands.Context):
    ...
    embed.add_field(name="Step 4", value="Run `!djoin <server_id>` to pull members", inline=False)
...
@bot.command(name="djoin", aliases=["farm", "join"])
@commands.guild_only()
async def djoin_command(ctx: commands.Context, server_id: str):
    limit = 0
    matched_role = None
    ...
    status_embed = discord.Embed(
        title="🔄 Farming...",
        description=f"Pulling members into **{target_guild.name}**\nTier: **{matched_role.title()}** (cap {limit})",
        color=discord.Color.orange()
    )
    ...
    result = discord.Embed(
        title="✅ Farm complete",
        description=f"Pulled into **{target_guild.name}** on tier **{matched_role.title()}** (cap {limit}).",
        color=discord.Color.green()
    )
    result.add_field(name="✅ Added", value=str(added), inline=True)
    result.add_field(name="↪️ Already in", value=str(already_in), inline=True)
    result.add_field(name="❌ Failed", value=str(failed), inline=True)
    result.set_footer(text="MemberFlow • Verify System")
...
async def slash_help(interaction: discord.Interaction):
    ...
    embed.add_field(name="Main", value="`/auth` `/addbot` `!djoin` `/pool`", inline=False)
...
@tree.command(name="claim", description="Claim the Members role (must be authorized)")
async def slash_claim(interaction: discord.Interaction):
    ...
    if not token:
        return await interaction.response.send_message(
            embed=discord.Embed(title="❌ Not Authorized", description="Use `/auth` first.", color=discord.Color.red()),
            ephemeral=True
        )

    role = interaction.guild.get_role(AUTO_ROLE_ID)
    if role is None:
        role = discord.utils.find(lambda r: r.name.lower() == "members", interaction.guild.roles)
    
    if role is None:
        return await interaction.response.send_message("❌ Members role not found.", ephemeral=True)

    if role in interaction.user.roles:
        return await interaction.response.send_message(
            embed=discord.Embed(title="✅ Already Have Role", description=f"You already have {role.mention}.", color=discord.Color.green()),
            ephemeral=True
        )

    bot_member = interaction.guild.get_member(bot.user.id)
    if bot_member is None or not bot_member.guild_permissions.manage_roles:
        return await interaction.response.send_message("❌ Bot needs Manage Roles permission.", ephemeral=True)

    if role >= bot_member.top_role:
        return await interaction.response.send_message(
            f"❌ Bot role must be **above** {role.mention} in Server Settings → Roles.",
            ephemeral=True
        )

    try:
        await interaction.user.add_roles(role, reason="Claimed via /claim")
        await interaction.response.send_message(
            embed=discord.Embed(title="✅ Role Claimed", description=f"You now have {role.mention}!", color=discord.Color.green())
        )
    except Exception as e:
...
@app_commands.choices(name=[
    app_commands.Choice(name="Auth / Verify Message", value="auth"),
    app_commands.Choice(name="Help Command", value="help"),
    app_commands.Choice(name="Tutorial", value="tutorial"),
    app_commands.Choice(name="Pool Message", value="pool"),
])
